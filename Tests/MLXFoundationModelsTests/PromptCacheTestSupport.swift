...
//
// Shared fixtures for `PromptCache` actor-level hybrid-checkpoint test
// suites (`PromptCacheHybridArchitectureTests`,
// `PromptCacheHybridEvictionScopeTests`): a minimal `LanguageModel` probe,
// a chunkable single-layer cache constructor, a hybrid-checkpoint
// constructor, and a `resolve()` convenience that hides the single-use
...
/// - Throws: Rethrows whatever `PromptCache.resolve` throws, ultimately from
///   `model.newCache(parameters:)`.
func resolveOnce(
