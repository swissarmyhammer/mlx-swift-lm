...
final class LoadWeightsTests: XCTestCase {
    ...
    func testLoadWeightsPreparesInferenceStateAfterInstallingParameters() async throws {
        let directory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }
    ...
    func testAsyncLoadWeightsInstallsTheWeightsFromTheGlobalQueue() async throws {
        let directory = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: directory) }

        try writeSidecarCheckpoint(in: directory)

        // loadWeights suspends the caller and runs the load on a global queue instead of
        // blocking a cooperative thread; the result must be the same as a direct load.
        let model = SidecarDeclaringModel()
        try await loadWeights(modelDirectory: directory, model: model)
    ...
    func testAsyncLoadWeightsPropagatesErrors() async throws {
        ...
        // the undeclared-sidecar failure must cross the global queue back to the caller
        let model = TwoLayerModel()
        do {
            try await loadWeights(modelDirectory: directory, model: model)
            XCTFail("expected the keyNotFound error from the undeclared sidecar")
        } catch {
            // expected
        }
    }
