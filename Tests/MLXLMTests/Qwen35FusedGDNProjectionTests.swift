...
final class Qwen35FusedGDNProjectionTests: XCTestCase {
    ...
    /// ```
    func testRealCheckpointBenchmark() async throws {
        guard
        ...
        func loadModel(fused: Bool) async throws -> Qwen35Model {
        ...
        let baselineModel = try await loadModel(fused: false)
        let fusedModel = try await loadModel(fused: true)
