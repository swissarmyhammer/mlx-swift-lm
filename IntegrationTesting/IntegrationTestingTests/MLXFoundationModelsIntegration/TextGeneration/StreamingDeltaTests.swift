        ...
        for try await event in stream {
            if let response = reflectedChannelPayload(
                ...
                reflectedChannelPayload(
                    of: response.action, caseLabel: "appendText", as: LanguageModelExecutorGenerationChannel.TextFragment.self) != nil
            {
                deltaCount += 1
            }
        }
        ...
        for try await event in stream {
            if let response = reflectedChannelPayload(
                ...
                let delta = reflectedChannelPayload(
                    of: response.action, caseLabel: "appendText", as: LanguageModelExecutorGenerationChannel.TextFragment.self)
            {
                text += delta.content
            }
        }
