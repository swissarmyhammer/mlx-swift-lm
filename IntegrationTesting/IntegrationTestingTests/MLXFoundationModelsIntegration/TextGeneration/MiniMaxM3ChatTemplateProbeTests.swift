...
/// Renders MiniMax-M3's real `chat_template.jinja` (fetched from
/// `mlx-community/MiniMax-M3-4bit` while implementing kanban ^ayw1xee, 2026)
/// through swift-transformers' Jinja engine (the `swift-jinja` package)
/// directly -- catching engine gaps (macros, namespace-token string
/// concatenation, `raise_exception`) before the real-weights integration
/// test in this same directory ever downloads a byte.
///
/// Drives `Jinja.Template` directly rather than a full
/// `Tokenizers.PreTrainedTokenizer`: the tokenizer wrapper additionally
/// requires a vocab/`tokenizer.json`, which this probe does not need --
/// `applyChatTemplate` itself is just `Template(_:).render(context:)` under
/// the hood (see `PreTrainedTokenizer.applyChatTemplate` in swift-transformers).
/// Mirrors `ApplyChatTemplateProbeTests`'s "renders without crashing" contract.
@Suite(.timeLimit(.minutes(1)))
struct MiniMaxM3ChatTemplateProbeTests {

    /// The real `chat_template.jinja` from `mlx-community/MiniMax-M3-4bit`,
    /// checked in verbatim.
    private static let chatTemplateFixture = #"""
...
"""#
    ...
    private static func makeTemplate() throws -> Template {
        try Template(Self.chatTemplateFixture, with: .init(lstripBlocks: true, trimBlocks: true))
    }

    /// An OpenAI-style tool spec, the shape `render_tool_namespace` expects
    ...
    /// A completed system + user + tool-call + tool-result + assistant
    /// round, mirroring `TranscriptConverterTests.completedToolRoundEntries`
    /// once rendered through `.minimaxM3`'s structured `tool_calls` shape.
    /// `role: "root"` is M3's highest-priority system slot (see the
    /// template's role-mapping comment).
    private static let systemUserToolMessages: [[String: Any]] = [
        ...
        ["role": "assistant", "content": "It's sunny and 22\u{00b0}C in Tokyo."],
    ]

    @Test
    func rendersSystemUserToolConversationWithoutThrowing() throws {
        let template = try Self.makeTemplate()
        let context: [String: Value] = [
            "messages": .array(try Self.systemUserToolMessages.map { try Value(any: $0) }),
            "tools": .array([try Value(any: Self.weatherTool)]),
            "add_generation_prompt": .boolean(true),
        ]

        let rendered = try template.render(context)

        // The namespaced tool_call wrapper and the invoked tool's name and
        // argument value must all appear in the rendered prompt -- the
        // `tool_calls` branch and the `to_xml` macro both fired correctly.
        #expect(rendered.contains("<tool_call>"))
        #expect(rendered.contains("get_weather"))
        #expect(rendered.contains("Tokyo"))
    }

    @Test
    func rendersWithoutToolsOrThinkingMode() throws {
        let template = try Self.makeTemplate()
    ...
    /// Forcing `thinking_mode` primes the generation prompt with the
    /// matching delimiter -- `.templateStringFlag`'s on/off toggle actually
    /// reaches the rendered prompt, matching `ReasoningConfig`'s
    /// `minimaxM3ThinkConfig` contract.
    @Test
    func honorsThinkingModeToggle() throws {
        let template = try Self.makeTemplate()
        let messages: [[String: Any]] = [
            ["role": "user", "content": "2+2="]
        ]
        let expectedSuffixes: [(mode: String, suffix: String)] = [
            ("enabled", "<mm:think>"),
            ("disabled", "</mm:think>"),
        ]
        for (mode, expectedSuffix) in expectedSuffixes {
            let context: [String: Value] = [
                "messages": .array(try messages.map { try Value(any: $0) }),
                "add_generation_prompt": .boolean(true),
                "thinking_mode": .string(mode),
            ]
            let rendered = try template.render(context)
            #expect(
                rendered.trimmingCharacters(in: .whitespacesAndNewlines).hasSuffix(expectedSuffix),
                "thinking_mode=\(mode) should end the rendered prompt with \(expectedSuffix), got: ...\(rendered.suffix(60))"
            )
        }
    }

    /// `thinking_mode: "adaptive"` (or leaving it unset) primes nothing --
    /// the model decides for itself, mirroring `defaultValue: "adaptive"` in
    /// `ReasoningConfig`'s `minimaxM3ThinkConfig`.
    @Test
    func adaptiveThinkingModeDoesNotPrimeGenerationPrompt() throws {
        let template = try Self.makeTemplate()
        let messages: [[String: Any]] = [
            ["role": "user", "content": "2+2="]
        ]
        let context: [String: Value] = [
            "messages": .array(try messages.map { try Value(any: $0) }),
            "add_generation_prompt": .boolean(true),
            "thinking_mode": .string("adaptive"),
        ]
        let rendered = try template.render(context)
        let trimmed = rendered.trimmingCharacters(in: .whitespacesAndNewlines)
        #expect(!trimmed.hasSuffix("<mm:think>"))
        #expect(!trimmed.hasSuffix("</mm:think>"))
    }
}
