...
package struct PromptCacheTurn: Sendable {
    ...
    package var canSplitPreparedMedia: Bool = false
}
...
package func reconcilePromptCache(
    ...
    case .appendMediaSuffix:
        // Only the model can split a prepared input, and this function has no
        // prepared input. The turn it builds carries no media, thus the
        // standard rule does not make this decision; a caller that gets it
        // from a protocol rule must build new caches.
        return nil
