...
- actor: claude-code
  id: 01kycmy0m4zbye65qckvk3x8fs
  text: |-
    Real-weights verification 2026-07-25: MiniMaxM3CoherenceIntegrationTests.minimax_m3() genuinely PASSED against the real mlx-community/MiniMax-M3-4bit checkpoint (225GB, full download completed) — first two attempts hit a reproducible ~8-connection WiFi timeout wave partway through the download (at 71GB and 172GB respectively) which the test's catch-and-skip design silently converted into false-positive passes without ever loading the model; the third attempt completed the download with zero timeouts and genuinely loaded the model ("Loaded VLM: mlx-community/MiniMax-M3-4bit" in the log, no "Skipping" line). Confirmed: MoE gate quantizes at 8-bit, expert switch_mlp.gate_up_proj quantizes at 4-bit affine, and "2+2=" produces a response containing "4". Total test time ~19 minutes post-download (model load + Metal kernel JIT + generation). This is the first genuine end-to-end real-weights confirmation of the MiniMax-M3 chain (config decode, language_model.-prefixed sanitize, MoE/swigluoai, dense attention, factory registration, text generation).

    Caveat for future runs: the false-positive skip-on-load-failure pattern is a real gap in this test's design — a transient network failure during the multi-hour download is indistinguishable from a genuine "no checkpoint available" skip in the reported PASS/skip output. Consider tightening this (e.g. distinguish "never attempted" from "attempted and failed" in the skip message, or fail loudly on a load error that isn't a clean 404/missing-file) if this test starts being trusted for CI signal rather than manual verification.
  timestamp: 2026-07-25T12:45:19.876603+00:00
depends_on:
