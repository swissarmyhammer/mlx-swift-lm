...
---
## What
NOTE: this fix lives on the `mlx-foundationmodels` branch of this repo (not `main`) — the local checkout used to discover this is on `main` and does not have `Libraries/MLXFoundationModels/` at all; check out/base work on `origin/mlx-foundationmodels` (the branch `FoundationModelsRouter`'s `Package.swift` pins as a dependency, per `.package(url: \"https://github.com/swissarmyhammer/mlx-swift-lm\", branch: \"mlx-foundationmodels\")`).
...
## Tests
- [ ] A new or updated test in this target's existing tool-calling test suite that registers two distinct tools with a `LanguageModelSession`, scripts/stubs the model to call the first tool, then asserts the session's second round is still capable of invoking the second tool (not silently downgraded to unconstrained text) — the regression test for this exact bug.
