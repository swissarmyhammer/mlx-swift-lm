...
- actor: claude-code
  id: 01kzywcmasypmezffhawa0mrpb
  text: |-
    ### Closed — card `^2ajc82t` was abandoned, thus these 9 tests have no port to guard

    The user wrote Qwen off for the time being on 2026-08-14 and dropped the prompt-cache port. These 9 tests were the specification for that port, thus nothing is left to decide.

    **They stay where they are:** `Tests/MLXFoundationModelsTests/PromptCacheHybridExecutorTests.swift.disabled`. The `.disabled` suffix keeps SwiftPM from reading them, thus they cost no build time and break nothing. `Package.swift` names them nowhere.

    They are kept rather than deleted for the same reason `PromptCache.swift` is kept: the user said "for the time being". A later reader who takes the port up again gets the specification without recovering a file.

    The 21 tests of the `PromptCache` actor stay live in `PromptCacheHybridArchitectureTests.swift`, and they pass.

    Delete this file when somebody decides Qwen caching is not coming back.
  timestamp: 2026-08-14T00:57:40.441022+00:00
